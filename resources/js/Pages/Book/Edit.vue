...
const form = useForm({
    ...
    room_id: props.book.room_id || '',
    start_date: props.book.start_date || '',
    end_date: props.book.end_date || '',
    start_hour: props.book.start_hour || '',
    finish_hour: props.book.finish_hour || '',
    notes: props.book.notes || '',
    isMultipleDate: props.book.end_date == null? false : true,
});
...
const submit = () => {
    form.patch(route('books.update', props.organization.id), {
        ...
        onError: (errors) => {
            dataStore.setAlertError("Gagal memperbarui pemesanan ruangan!")
        }
    });
};
</script>
...
<template>
    <Head title="Edit Pesanan Ruangan" />
